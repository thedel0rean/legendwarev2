...
namespace ns_menu
{
	...
	void add_check_box(sol::this_state s, std::string name)
	{
	...
	void add_color_picker(sol::this_state s, std::string name)
	{
		auto script = get_current_script(s);
		auto script_id = c_lua::get().get_script_id(script);
	...
	{
		if (!hooks::menu_open && stored_values.find(name) != stored_values.end())
...
namespace ns_render
{
	Vector world_to_screen(Vector world)
	{
		Vector screen;
	...
	int get_text_width(vgui::HFont font, std::string text)
	{
		return render::get().text_width(font, text.c_str());
	}

	vgui::HFont create_font(std::string name, float size, float weight, std::optional <bool> antialias, std::optional <bool> dropshadow, std::optional <bool> outline)
	{
...
void c_lua::initialize()
{
	...
	cmd[crypt_str("get_user_cmd")] = ns_cmd::get_send_packet;
	cmd[crypt_str("get_send_packet")] = ns_cmd::get_choke;
...
void c_lua::load_script(int id)
{
	...
	lua.script_file(path, [](lua_State*, sol::protected_function_result result)
		{
			if (!result.valid())
			{
