...
void antiaim::create_move(CUserCmd* m_pcmd)
{
	...
	if ((type == ANTIAIM_LEGIT ? g_cfg.antiaim.desync : g_cfg.antiaim.type[type].desync) && (g_ctx.globals.exploits || (type == ANTIAIM_LEGIT ? !g_cfg.antiaim.legit_lby_type : !g_cfg.antiaim.lby_type)) && !g_cfg.misc.fast_stop && (!g_ctx.globals.weapon->is_grenade() || !(m_pcmd->m_buttons & IN_ATTACK) && !(m_pcmd->m_buttons & IN_ATTACK2)) && engineprediction::get().backup_data.velocity.Length2D() <= 20.0f) //-V648
	{
...
float antiaim::get_yaw(CUserCmd* m_pcmd)
{
	...
	{	
		...
		else if (g_cfg.antiaim.desync == 2 && g_ctx.send_packet)
			flip = !flip;

		desync_angle = g_ctx.local()->get_max_desync_delta();
	...
	else
	{
		...
		{
		case 1:
			yaw_angle = jitter_side ? (float)g_cfg.antiaim.type[type].range * -0.5f : (float)g_cfg.antiaim.type[type].range * 0.5f;
			break;
		case 2:
		{
			if (flip)
		...
		{
			if (g_cfg.antiaim.type[type].desync == 2 && g_ctx.send_packet)
				flip = !flip;

			auto desync_delta = g_ctx.local()->get_max_desync_delta();
			...
			if (type != ANTIAIM_STAND && type != ANTIAIM_LEGIT || !g_cfg.antiaim.lby_type || g_ctx.globals.exploits)
			{
				if (!flip)
	...
	if (should_break_lby(m_pcmd, lby_type))
	{
		...
		auto move = g_ctx.local()->m_vecViewOffset().z <= 63.0f ? 3.3f : 1.1f;

		if (switch_move)
			m_pcmd->m_forwardmove = move;
		else
			m_pcmd->m_forwardmove = -move;
...
bool antiaim::condition(CUserCmd* m_pcmd, bool dynamic_check)
{
	...
	if (m_pcmd->m_buttons & IN_ATTACK && g_ctx.globals.weapon->m_iItemDefinitionIndex() != WEAPON_REVOLVER && !g_ctx.globals.weapon->is_non_aim())
		return true;
	...
	if ((m_pcmd->m_buttons & IN_ATTACK || m_pcmd->m_buttons & IN_ATTACK2) && g_ctx.globals.weapon->is_knife())
		return true;
	...
	if (dynamic_check && g_ctx.globals.weapon->is_grenade() && g_ctx.globals.weapon->m_fThrowTime())
		return true;
...
{	
	if (!lby_type || g_ctx.globals.exploits)
		return false;
	...
	else if (m_globals()->m_curtime + TICKS_TO_TIME(1) > g_ctx.globals.next_lby_update)
		g_ctx.globals.next_tick_lby_update = true;
