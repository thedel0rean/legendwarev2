...
void misc::AutoCrouch(CUserCmd* cmd)
{
	...
	auto crouch = m_clientstate()->m_iChockedCommands >= 7;
...
void misc::AsusWalls()
{
	...
	{
		auto pMaterial = materialsystem->GetMaterial(i);

		if (!pMaterial)
			continue;

		if (strstr(pMaterial->GetTextureGroupName(), crypt_str("StaticProp")))
		{
			if (g_cfg.esp.asus_props && g_cfg.player.enable)
				pMaterial->AlphaModulate(g_cfg.esp.asus_props_amount);
			else
...
void misc::NightmodeFix()
{
	...
	static auto setting = g_cfg.player.enable && g_cfg.esp.nightmode;

	if (setting != (g_cfg.player.enable && g_cfg.esp.nightmode))
	{
		setting = g_cfg.player.enable && g_cfg.esp.nightmode;
		g_ctx.globals.change_materials = true;
	}

	static auto value = g_cfg.esp.nightmode_value;

	if (value != g_cfg.esp.nightmode_value)
	{
		value = g_cfg.esp.nightmode_value;
		g_ctx.globals.change_materials = true;
	}

	static auto setting_props = g_cfg.player.enable && g_cfg.esp.asus_props;

	if (setting_props != (g_cfg.player.enable && g_cfg.esp.asus_props))
	{
		setting_props = g_cfg.player.enable && g_cfg.esp.asus_props;
		g_ctx.globals.change_materials = true;
	}

	static auto value_props = g_cfg.esp.asus_props_amount;

	if (value_props != g_cfg.esp.asus_props_amount) //-V550
	{
		value_props = g_cfg.esp.asus_props_amount;
		g_ctx.globals.change_materials = true;
	}
}
...
void misc::ragdolls()
{
	...
	static auto cl_ragdoll_gravity = m_cvar()->FindVar(crypt_str("cl_ragdoll_gravity"));

	if (g_cfg.misc.ragdolls)
	{
		forced_convar = true;

		if (cl_ragdoll_gravity->GetFloat() != -750.0f) //-V550
			cl_ragdoll_gravity->SetValue(-750.0f);
	}
...
void misc::fast_stop(CUserCmd* m_pcmd)
{
	...
	if (!((antiaim::get().type == ANTIAIM_LEGIT ? g_cfg.antiaim.desync : g_cfg.antiaim.type[antiaim::get().type].desync) && (g_ctx.globals.exploits || (antiaim::get().type == ANTIAIM_LEGIT ? !g_cfg.antiaim.legit_lby_type : !g_cfg.antiaim.lby_type)) && (!g_ctx.globals.weapon->is_grenade() || !(m_pcmd->m_buttons & IN_ATTACK) && !(m_pcmd->m_buttons & IN_ATTACK2))) || antiaim::get().condition(m_pcmd)) //-V648
	{
...
void misc::spectators_list()
{
	...
	for (int i = 1; i < m_globals()->m_maxclients; i++)
	{
		...
		if (e->m_hObserverTarget() != g_ctx.local())
			continue;
...
bool misc::double_tap(CUserCmd* m_pcmd)
{
	...
	{
		if (g_ctx.send_packet && m_pcmd->m_buttons & IN_ATTACK && g_ctx.globals.weapon->can_shift_tickbase(14))
		{
	...
	{
		static auto did_shift_before = false;

		auto can_shift = g_ctx.globals.weapon->can_shift_tickbase(11);
		auto can_shift_next_tick = g_ctx.globals.weapon->can_shift_tickbase(g_ctx.globals.next_tickbase_shift + 1);

		if (can_shift || !did_shift_before && !can_shift_next_tick) //-V648
			g_ctx.globals.next_tickbase_shift = 11;
		else
			g_ctx.globals.next_tickbase_shift = 0;

		if (g_ctx.send_packet)
		{
			if (m_pcmd->m_buttons & IN_ATTACK && g_ctx.globals.next_tickbase_shift)
			{
				...
				g_ctx.globals.double_tap_fire = true;
			}

			did_shift_before = g_ctx.globals.next_tickbase_shift;
		}
	}
...
void misc::hide_shots(CUserCmd* m_pcmd, bool should_work)
{
	...
	g_ctx.globals.next_tickbase_shift = 9;
