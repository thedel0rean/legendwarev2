...
void eventlogs::events(IGameEvent* event) 
{
	...
	static char* hasdefusekit[2] = 
	{
		crypt_str("without defuse kit"),
		crypt_str("with defuse kit")
	};
	...
	if (g_cfg.misc.events_to_log[EVENTLOG_BOMB] && !strcmp(event->GetName(), crypt_str("bomb_begindefuse")))
	{
		...
		std::stringstream ss;
		ss << userid_info.szName << crypt_str(" has began defusing the bomb ") << hasdefusekit[event->GetBool(crypt_str("haskit"))];
...
#endif
#else
		m_cvar()->ConsoleColorPrintf(g_cfg.misc.log_color, crypt_str("[ LEGENDWARE ALPHA ] "));
#endif
