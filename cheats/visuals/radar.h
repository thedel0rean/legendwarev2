...
private:
	...
	bool RenderMap()
	{
		...
		ImGui::Begin("##radar", &open, ImVec2(256, 256), 1.f, ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoScrollWithMouse | ImGuiWindowFlags_NoTitleBar);
		...
		if (local_player)
		{
			for (auto i = 1; i <= m_entitylist()->GetHighestEntityIndex(); i++)
			{
				...
				else if (client_class->m_ClassID == CPlantedC4)
				{
					Vector bomb_pos;
