...
void playeresp::draw_weapon(player_t* m_entity, const Box& box, bool space)
{
	...
	{
		render::get().text(fonts[ESP], box.x + box.w / 2, pos, color, HFONT_CENTERED_X, weapon->get_name());
		pos += 11;
	}
...
void playeresp::draw_flags(player_t* e, const Box& box)
{
	...
	if (g_cfg.player.type[type].flags[FLAGS_FAKEDUCKING])
	{
		...
		{
			static int stored_tick;
			static int crouched_ticks[65];

			auto m_flDuckAmount = e->get_animation_state()->m_fDuckAmount;
			auto m_flDuckSpeed = e->m_flDuckSpeed();

			if (m_flDuckSpeed && m_flDuckAmount)
			{
				if (m_flDuckSpeed == 8.0f && m_flDuckAmount < 0.9f && m_flDuckAmount > 0.5f) //-V550
				{
		...
		if (fakeducking() && e->m_fFlags() & FL_ONGROUND && !e->get_animation_state()->m_bInHitGroundAnimation)
		{
...
void playeresp::draw_multi_points(player_t* e)
{
	...
	auto hitboxes = aimbot::get().hitboxes_from_vector(e, g_cfg.ragebot.weapon[g_ctx.globals.current_weapon].hitscan, true);

	for (auto current : hitboxes)
	{
		std::vector <aimbot::point> points;
		aimbot::get().getpoints(e, current, points, false);

		for (auto point : points)
		{
			Vector screen;

			if (!math::world_to_screen(point.position, screen))
				continue;
