			...
			if (e == g_ctx.local())
			{
				...
				for (auto& current : impacts)
				{
					if (e == current.e)
					{
						...
						current.time = m_globals()->m_curtime + TICKS_TO_TIME(1);
					}
				}
				...
				if (new_record)
					impacts.push_back(
						impact_data
						{
							e,
							position,
							m_globals()->m_curtime + TICKS_TO_TIME(1)
						});
			}
...
void bullettracers::draw_beams()
{
	...
	while (!impacts.empty())
	{
		...
		if (TICKS_TO_TIME(m_globals()->m_curtime) > TICKS_TO_TIME(impacts.begin()->time))
		{
			...
			draw_beam(impacts.begin()->e == g_ctx.local(), impacts.begin()->e == g_ctx.local() ? aimbot::get().shoot_poisiton : impacts.begin()->e->get_shoot_position(), impacts.begin()->impact_position, color);
			impacts.erase(impacts.begin());
			continue;
		}
