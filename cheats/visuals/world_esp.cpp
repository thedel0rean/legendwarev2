...
void worldesp::paint_traverse()
{
	...
	for (int i = 1; i <= m_entitylist()->GetHighestEntityIndex(); i++)  //-V807
	{
		...
		default:
			...
			if (client_class->m_ClassID == CAK47 || client_class->m_ClassID == CDEagle || client_class->m_ClassID >= CWeaponAug && client_class->m_ClassID <= CWeaponZoneRepulsor || client_class->m_ClassID == CBaseCSGrenadeProjectile || client_class->m_ClassID == CSmokeGrenadeProjectile || client_class->m_ClassID == CSensorGrenadeProjectile || client_class->m_ClassID == CMolotovProjectile || client_class->m_ClassID == CDecoyProjectile) //-V648
				dropped_weapons(e);
...
void worldesp::grenade_projectiles(entity_t* entity)
{
	...
	if (client_class->m_ClassID == CBaseCSGrenadeProjectile || client_class->m_ClassID == CSmokeGrenadeProjectile || client_class->m_ClassID == CSensorGrenadeProjectile || client_class->m_ClassID == CMolotovProjectile || client_class->m_ClassID == CDecoyProjectile)
	{
	...
	else
	{
		...
		if (name.find("thrown") != std::string::npos || name.find("dropped") != std::string::npos)
		{
			...
			if (util::get_bbox(weapon, box, false))
			{
				...
				if (g_cfg.esp.weapon[WEAPON_BOX])
				{
					...
					if (g_cfg.esp.weapon[WEAPON_TEXT])
						render::get().text(fonts[ESP], box.x + box.w / 2, box.y + box.h + 2, g_cfg.esp.weapon_color, HFONT_CENTERED_X, weapon->get_name());
				...
				else
				{
					...
					if (g_cfg.esp.weapon[WEAPON_TEXT])
						render::get().text(fonts[ESP], box.x + box.w / 2, box.y + box.h / 2 + 6, g_cfg.esp.weapon_color, HFONT_CENTERED_X, weapon->get_name());
