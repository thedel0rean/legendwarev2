...
item_setting* get_by_definition_index(const int definition_index)
{
	...
	{
		return g_cfg.skins.enable && e.itemId == definition_index;
	});
...
void C_ConfigManager::setup()
{
	...
	setup_item(&g_cfg.legitbot.deathmatch, false, crypt_str("Legitbot.deathmatch"));
	setup_item(&g_cfg.legitbot.autopistol, false, crypt_str("Legitbot.autopistol"));
	setup_item(&g_cfg.legitbot.autowall, false, crypt_str("Legitbot.autowall"));
	setup_item(&g_cfg.legitbot.silent, false, crypt_str("Legitbot.silent"));
	setup_item(&g_cfg.legitbot.autofire, false, crypt_str("Legitbot.autofire"));
	setup_item(&g_cfg.legitbot.on_key, false, crypt_str("Legitbot.on_key"));
	setup_item(&g_cfg.legitbot.autostop, false, crypt_str("Legitbot.autostop"));
	setup_item(&g_cfg.legitbot.only_in_zoom, false, crypt_str("Legitbot.only_in_zoom"));
	...
	for (auto i = 0; i < 5; i++)
	{
		setup_item(&g_cfg.legitbot.weapon[i].smoke_check, false, std::to_string(i) + crypt_str("Legitbot.smoke_check"));
		setup_item(&g_cfg.legitbot.weapon[i].flash_check, false, std::to_string(i) + crypt_str("Legitbot.flash_check"));
		setup_item(&g_cfg.legitbot.weapon[i].jump_check, false, std::to_string(i) + crypt_str("Legitbot.jump_check"));
		setup_item(&g_cfg.legitbot.weapon[i].rcs, false, std::to_string(i) + crypt_str("Legitbot.rcs"));
		setup_item(&g_cfg.legitbot.weapon[i].rcs_fov_enabled, false, std::to_string(i) + crypt_str("Legitbot.rcs_fov_enabled"));
		setup_item(&g_cfg.legitbot.weapon[i].rcs_smooth_enabled, false, std::to_string(i) + crypt_str("Legitbot.rcs_smooth_enabled"));
		setup_item(&g_cfg.legitbot.weapon[i].aim_type, 0, std::to_string(i) + crypt_str("Legitbot.aim_type"));
		...
		setup_item(&g_cfg.legitbot.weapon[i].hitbox, 0, std::to_string(i) + crypt_str("Legitbot.hitbox"));
		setup_item(&g_cfg.legitbot.weapon[i].fov, 1.0f, std::to_string(i) + crypt_str("Legitbot.fov"));
		setup_item(&g_cfg.legitbot.weapon[i].silent_fov, 1.0f, std::to_string(i) + crypt_str("Legitbot.silent_fov"));
		setup_item(&g_cfg.legitbot.weapon[i].rcs_fov, 1.0f, std::to_string(i) + crypt_str("Legitbot.rcs_fov"));
		setup_item(&g_cfg.legitbot.weapon[i].smooth, 1.0f, std::to_string(i) + crypt_str("Legitbot.smooth"));
		setup_item(&g_cfg.legitbot.weapon[i].rcs_smooth, 1.0f, std::to_string(i) + crypt_str("Legitbot.rcs_smooth"));
		setup_item(&g_cfg.legitbot.weapon[i].shot_delay, 0, std::to_string(i) + crypt_str("Legitbot.shot_delay"));
		setup_item(&g_cfg.legitbot.weapon[i].kill_delay, 0, std::to_string(i) + crypt_str("Legitbot.kill_delay"));
		setup_item(&g_cfg.legitbot.weapon[i].rcs_x, 0, std::to_string(i) + crypt_str("Legitbot.rcs_x"));
		setup_item(&g_cfg.legitbot.weapon[i].rcs_y, 0, std::to_string(i) + crypt_str("Legitbot.rcs_y"));
		setup_item(&g_cfg.legitbot.weapon[i].rcs_start, 1, std::to_string(i) + crypt_str("Legitbot.rcs_start"));
		setup_item(&g_cfg.legitbot.weapon[i].min_damage, 1, std::to_string(i) + crypt_str("Legitbot.min_damage"));
	}
	...
	setup_item(&g_cfg.ragebot.instant, false, crypt_str("Ragebot.instant"));
	...
	setup_item(&g_cfg.ragebot.baim_key, key_bind(HOLD), crypt_str("Ragebot.baim_key"));
	...
	for (auto i = 0; i < 8; i++)
	{
		...
		setup_item(&g_cfg.ragebot.weapon[i].autostop_modifiers, 5, std::to_string(i) + crypt_str("Ragebot.autostop_modifiers"));
		setup_item(&g_cfg.ragebot.weapon[i].baim_level, 0, std::to_string(i) + crypt_str("Ragebot.baim_level"));
		setup_item(&g_cfg.ragebot.weapon[i].baim_settings, 5, std::to_string(i) + crypt_str("Ragebot.baim_conditions"));
		setup_item(&g_cfg.ragebot.weapon[i].skip_hitchance_if_low_inaccuracy, false, std::to_string(i) + crypt_str("Ragebot.skip_hitchance_if_low_inaccuracy"));
		...
		setup_item(&g_cfg.ragebot.weapon[i].hitscan, 7, std::to_string(i) + crypt_str("Ragebot.hitscan"));
		setup_item(&g_cfg.ragebot.weapon[i].safe_body_points, false, std::to_string(i) + crypt_str("Ragebot.safe_body_points"));
		setup_item(&g_cfg.ragebot.weapon[i].pointscale_head, 0.0f, std::to_string(i) + crypt_str("Ragebot.pointscale_head"));
		setup_item(&g_cfg.ragebot.weapon[i].pointscale_chest, 0.0f, std::to_string(i) + crypt_str("Ragebot.pointscale_chest"));
		setup_item(&g_cfg.ragebot.weapon[i].pointscale_stomach, 0.0f, std::to_string(i) + crypt_str("Ragebot.pointscale_stomach"));
		setup_item(&g_cfg.ragebot.weapon[i].pointscale_pelvis, 0.0f, std::to_string(i) + crypt_str("Ragebot.pointscale_pelvis"));
		setup_item(&g_cfg.ragebot.weapon[i].pointscale_legs, 0.0f, std::to_string(i) + crypt_str("Ragebot.pointscale_legs"));
	}
	...
	setup_item(&g_cfg.antiaim.flip_indicator_color, Color::White, crypt_str("Antiaim.flip_indicator_color"));
	...
	setup_item(&g_cfg.player.arrows_color, Color::White, crypt_str("Player.arrows_color"));
	...
	setup_item(&g_cfg.player.show_multi_points_color, Color::White, crypt_str("Player.show_multi_points_color"));
	setup_item(&g_cfg.player.lag_hitbox, false, crypt_str("Player.lag_hitbox"));
	setup_item(&g_cfg.player.lag_hitbox_color, Color::White, crypt_str("Player.lag_hitbox_color"));
	...
	setup_item(&g_cfg.player.fake_chams_color, Color::White, crypt_str("Player.fake_chams_color"));
	...
	setup_item(&g_cfg.player.fake_double_material_color, Color::White, crypt_str("Player.fake_double_material_color"));
	setup_item(&g_cfg.player.fake_animated_material, false, crypt_str("Player.fake_animated_material"));
	setup_item(&g_cfg.player.fake_animated_material_color, Color::White, crypt_str("Player.fake_animated_material_color"));
	...
	setup_item(&g_cfg.player.backtrack_chams_color, Color::White, crypt_str("Player.backtrack_chams_color"));
	...
	for (auto i = 0; i < 3; i++)
	{
		...
		setup_item(&g_cfg.player.type[i].box_color, Color::White, std::to_string(i) + crypt_str("Player.box_color"));
		setup_item(&g_cfg.player.type[i].name, false, std::to_string(i) + crypt_str("Player.name"));
		setup_item(&g_cfg.player.type[i].name_color, Color::White, std::to_string(i) + crypt_str("Player.name_color"));
		...
		setup_item(&g_cfg.player.type[i].health_color, Color::White, std::to_string(i) + crypt_str("Player.health_color"));
		setup_item(&g_cfg.player.type[i].weapon, 2, std::to_string(i) + crypt_str("Player.weapon"));
		setup_item(&g_cfg.player.type[i].weapon_color, Color::White, std::to_string(i) + crypt_str("Player.weapon_bar_color"));
		setup_item(&g_cfg.player.type[i].skeleton, false, std::to_string(i) + crypt_str("Player.skeleton"));
		setup_item(&g_cfg.player.type[i].skeleton_color, Color::White, std::to_string(i) + crypt_str("Player.skeleton_color"));
		setup_item(&g_cfg.player.type[i].ammo, false, std::to_string(i) + crypt_str("Player.ammo"));
		setup_item(&g_cfg.player.type[i].ammobar_color, Color::White, std::to_string(i) + crypt_str("Player.ammobar_color"));
		setup_item(&g_cfg.player.type[i].snap_lines, false, std::to_string(i) + crypt_str("Player.snap_lines"));
		setup_item(&g_cfg.player.type[i].snap_lines_color, Color::White, std::to_string(i) + crypt_str("Player.snap_lines_color"));
		setup_item(&g_cfg.player.type[i].footsteps, false, std::to_string(i) + crypt_str("Player.footsteps"));
		setup_item(&g_cfg.player.type[i].footsteps_color, Color::White, std::to_string(i) + crypt_str("Player.footsteps_color"));
		...
		setup_item(&g_cfg.player.type[i].glow_color, Color::White, std::to_string(i) + crypt_str("Player.glow_color"));
		setup_item(&g_cfg.player.type[i].chams, 2, std::to_string(i) + crypt_str("Player.chams"));
		setup_item(&g_cfg.player.type[i].chams_color, Color::White, std::to_string(i) + crypt_str("Player.chams_color"));
		setup_item(&g_cfg.player.type[i].xqz_color, Color::White, std::to_string(i) + crypt_str("Player.xqz_color"));
		...
		setup_item(&g_cfg.player.type[i].double_material_color, Color::White, std::to_string(i) + crypt_str("Player.double_material_color"));
		setup_item(&g_cfg.player.type[i].animated_material, false, std::to_string(i) + crypt_str("Player.animated_material"));
		setup_item(&g_cfg.player.type[i].animated_material_color, Color::White, std::to_string(i) + crypt_str("Player.animated_material_color"));
		...
		setup_item(&g_cfg.player.type[i].ragdoll_chams_color, Color::White, std::to_string(i) + crypt_str("Player.ragdoll_chams_color"));
		setup_item(&g_cfg.player.type[i].flags, 8, std::to_string(i) + crypt_str("Player.esp_flags"));
	}

	setup_item(&g_cfg.esp.indicators, 7, crypt_str("Esp.indicators"));
	...
	setup_item(&g_cfg.esp.grenade_prediction_color, Color::White, crypt_str("Esp.grenade_prediction_color"));
	setup_item(&g_cfg.esp.grenade_prediction_tracer_color, Color::White, crypt_str("Esp.grenade_prediction_tracer_color"));
	setup_item(&g_cfg.esp.projectiles, false, crypt_str("Esp.projectiles"));
	setup_item(&g_cfg.esp.projectiles_color, Color::White, crypt_str("Esp.projectiles_color"));
	setup_item(&g_cfg.esp.molotov_timer, false, crypt_str("Esp.molotov_timer"));
	setup_item(&g_cfg.esp.molotov_timer_color, Color::White, crypt_str("Esp.molotov_timer_color"));
	...
	setup_item(&g_cfg.esp.nightmode_value, 1, crypt_str("Esp.nightmode_value"));
	...
	setup_item(&g_cfg.esp.skybox_color, Color::White, crypt_str("Esp.skybox_color"));
	setup_item(&g_cfg.esp.client_bullet_impacts, false, crypt_str("Esp.client_bullet_impacts"));
	setup_item(&g_cfg.esp.client_bullet_impacts_color, Color::White, crypt_str("Esp.client_bullet_impacts_color"));
	setup_item(&g_cfg.esp.server_bullet_impacts, false, crypt_str("Esp.server_bullet_impacts"));
	setup_item(&g_cfg.esp.server_bullet_impacts_color, Color::White, crypt_str("Esp.server_bullet_impacts_color"));
	setup_item(&g_cfg.esp.bullet_tracer, false, crypt_str("Esp.bullet_tracer"));
	setup_item(&g_cfg.esp.bullet_tracer_color, Color::White, crypt_str("Esp.bullet_tracer_color"));
	setup_item(&g_cfg.esp.enemy_bullet_tracer, false, crypt_str("Esp.enemy_bullet_tracer"));
	setup_item(&g_cfg.esp.enemy_bullet_tracer_color, Color::White, crypt_str("Esp.enemy_bullet_tracer_color"));
	setup_item(&g_cfg.esp.preserve_killfeed, false, crypt_str("Esp.preserve_killfeed"));
	setup_item(&g_cfg.esp.asus_props, false, crypt_str("Esp.asus_props"));
	setup_item(&g_cfg.esp.asus_props_amount, 1.0f, crypt_str("Esp.asus_props_amount"));
	setup_item(&g_cfg.esp.hitmarker, false, crypt_str("Esp.hitmarker"));
	...
	setup_item(&g_cfg.esp.arms_chams_color, Color::White, crypt_str("Esp.arms_chams_color"));
	setup_item(&g_cfg.esp.arms_double_material, false, crypt_str("Esp.arms_double_material"));
	setup_item(&g_cfg.esp.arms_double_material_color, Color::White, crypt_str("Esp.arms_double_material_color"));
	setup_item(&g_cfg.esp.arms_animated_material, false, crypt_str("Esp.arms_animated_material"));
	setup_item(&g_cfg.esp.arms_animated_material_color, Color::White, crypt_str("Esp.arms_animated_material_color"));
	...
	setup_item(&g_cfg.esp.weapon_chams_color, Color::White, crypt_str("Esp.weapon_chams_color"));
	setup_item(&g_cfg.esp.weapon_double_material, false, crypt_str("Esp.weapon_double_material"));
	setup_item(&g_cfg.esp.weapon_double_material_color, Color::White, crypt_str("Esp.weapon_double_material_color"));
	setup_item(&g_cfg.esp.weapon_animated_material, false, crypt_str("Esp.weapon_animated_material"));
	setup_item(&g_cfg.esp.weapon_animated_material_color, Color::White, crypt_str("Esp.weapon_animated_material_color"));
	...
	setup_item(&g_cfg.esp.show_spread_color, Color::White, crypt_str("Esp.show_spread_color"));
	...
	setup_item(&g_cfg.esp.fog_color, Color::White, crypt_str("Esp.fog_color"));
	setup_item(&g_cfg.esp.weapon, 6, crypt_str("Esp.dropped_weapon"));
	setup_item(&g_cfg.esp.box_color, Color::White, crypt_str("Esp.dropped_weapon_box_color"));
	setup_item(&g_cfg.esp.weapon_color, Color::White, crypt_str("Esp.dropped_weapon_color"));
	setup_item(&g_cfg.esp.weapon_glow_color, Color::White, crypt_str("Esp.dropped_weapon_glow_color"));
	setup_item(&g_cfg.esp.weapon_ammo_color, Color::White, crypt_str("Esp.dropped_weapon_ammo_color"));
	setup_item(&g_cfg.esp.grenade_esp, 4, crypt_str("Esp.grenade_esp"));
	setup_item(&g_cfg.esp.grenade_glow_color, Color::White, crypt_str("Esp.grenade_glow_color"));
	setup_item(&g_cfg.esp.grenade_box_color, Color::White, crypt_str("Esp.grenade_box_color"));

	setup_item(&g_cfg.misc.hold_firing_animation, false, crypt_str("Misc.hold_firing_animation"));
	...
	setup_item(&g_cfg.misc.teleport_exploit, key_bind(HOLD), crypt_str("Misc.teleport_exploit"));
	...
	setup_item(&g_cfg.misc.log_color, Color::White, crypt_str("Misc.log_color"));
	...
	setup_item(&g_cfg.skins.enable, false, crypt_str("Skins.enable"));
	...
	setup_item(&g_cfg.menu.menu_theme, Color(230, 60, 170), crypt_str("Menu.color"));
