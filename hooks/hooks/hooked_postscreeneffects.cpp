...
void __fastcall hooks::hooked_postscreeneffects(void* thisptr, void* edx, CViewSetup* setup)
{
	...
	if (!g_cfg.player.type[ENEMY].glow && !g_cfg.player.type[TEAM].glow && !g_cfg.player.type[LOCAL].glow)
		return original_fn(thisptr, setup);
	...
	for (auto i = 0; i < m_glow()->m_GlowObjectDefinitions.Count(); i++) //-V807
	{
		...
		else if (g_cfg.esp.weapon[WEAPON_GLOW] && (client_class->m_ClassID == CBaseCSGrenadeProjectile || client_class->m_ClassID == CSmokeGrenadeProjectile || client_class->m_ClassID == CSensorGrenadeProjectile || client_class->m_ClassID == CMolotovProjectile || client_class->m_ClassID == CDecoyProjectile || client_class->m_ClassID == CAK47 || client_class->m_ClassID == CDEagle || client_class->m_ClassID >= CWeaponAug && client_class->m_ClassID <= CWeaponXM1014)) //-V648
		{
			float color[4] =
			{
		...
		else if (g_cfg.esp.grenade_esp[GRENADE_GLOW])
		{
			...
			if (!strstr(name.c_str(), "thrown") && !strstr(name.c_str(), "dropped"))
				continue;
