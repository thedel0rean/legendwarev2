...
#include "..\..\cheats\lagcompensation\backtrack.h"
...
void __stdcall hooks::hooked_fsn(ClientFrameStage_t stage)
{
	...
	if (stage == FRAME_RENDER_END)
	{
		...
		if (g_cfg.player.enable && (g_cfg.esp.nightmode || g_cfg.esp.asus_props) && r_drawspecificstaticprop->GetBool())
			r_drawspecificstaticprop->SetValue(FALSE);
		else if ((!g_cfg.player.enable || !g_cfg.esp.nightmode && !g_cfg.esp.asus_props) && !r_drawspecificstaticprop->GetBool()) //-V648
			r_drawspecificstaticprop->SetValue(TRUE);
	...
	if (stage == FRAME_NET_UPDATE_END)
	{
		...
		else if (!g_ctx.shot.printed && g_ctx.shot.weapon_fired && TIME_TO_TICKS(m_globals()->m_curtime) > TIME_TO_TICKS(g_ctx.shot.shot_time) && !g_ctx.shot.player_hurt)
		{
			...
			if (g_ctx.shot.impact_hit)
			{
				...
				force_update = aimbot::get().target_id;

				if (g_ctx.globals.check_aa[aimbot::get().target_id] && g_ctx.globals.side[aimbot::get().target_id] == aimbot::get().target_side[aimbot::get().target_id])
				{
					g_ctx.globals.missed_shots[aimbot::get().target_id]++;
					auto resolve_index = g_ctx.globals.missed_shots[aimbot::get().target_id] % 2;

					if (g_ctx.globals.lby[aimbot::get().target_id])
						resolve_index = g_ctx.globals.missed_shots[aimbot::get().target_id] % 3;

					if (resolve_index == 1)
						g_ctx.globals.check_side_type[aimbot::get().target_id] = !g_ctx.globals.check_side_type[aimbot::get().target_id];

					if (resolve_index == 1 && g_ctx.globals.history_data[aimbot::get().target_id].type_normal)
						g_ctx.globals.history_data[aimbot::get().target_id].type_normal = 0;
					else if (resolve_index == 1)
						g_ctx.globals.history_data[aimbot::get().target_id].type_normal++;

					if (g_ctx.globals.missed_shots[aimbot::get().target_id] && (!resolve_index || resolve_index == 2) && g_ctx.globals.history_data[aimbot::get().target_id].type_normal)
						g_ctx.globals.history_data[aimbot::get().target_id].type_normal = 0;

					if (g_ctx.globals.history_data[aimbot::get().target_id].type_normal > 2)
						g_ctx.globals.history_data[aimbot::get().target_id].type_normal = 2;

					if (g_ctx.globals.lby[aimbot::get().target_id])
					{
						if (g_ctx.globals.history_data[aimbot::get().target_id].type_low_delta && resolve_index == 1)
							g_ctx.globals.history_data[aimbot::get().target_id].type_low_delta = false;
						else if (resolve_index == 1)
							g_ctx.globals.history_data[aimbot::get().target_id].type_low_delta = true;
					}
				}
				else if (!g_ctx.globals.check_aa[aimbot::get().target_id])
				{
					g_ctx.globals.check_aa[aimbot::get().target_id] = true;
					g_ctx.globals.check_aa_enable[aimbot::get().target_id] = false;
				}
			}
	...
	lagcompensation::get().fsn(stage, force_update);
	original_fn(stage);
	...
	static auto death_notice = util::FindHudElement <DWORD> (crypt_str("CCSGO_HudDeathNotice"));
