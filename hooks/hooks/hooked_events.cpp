...
void reset_history_data()
{
	for (auto i = 1; i < m_globals()->m_maxclients; i++)
	{
		...
		if (player_info.iSteamID != steam_ids[i]) 
		{
			steam_ids[i] = player_info.iSteamID; 
			g_ctx.globals.history_data[i].reset();
		}
	}
}
...
void C_HookedEvents::FireGameEvent(IGameEvent* event)
{
	...
	else if (!strcmp(event_name, crypt_str("weapon_fire")))
	{
		...
		if (g_ctx.globals.weapon_fired && user == m_engine()->GetLocalPlayer())
		{
			auto weapon = g_ctx.local()->m_hActiveWeapon().Get();

			if (!weapon->is_non_aim() && weapon->m_iItemDefinitionIndex() != WEAPON_TASER && aimbot::get().target_id != -1)
			{
	...
	else if (!strcmp(event_name, crypt_str("player_hurt")))
	{
		...
		if (attacker_id == m_engine()->GetLocalPlayer() && user_id != m_engine()->GetLocalPlayer())
		{
			...
			if (weapon_is_aim(weapon))
			{
				...
				{
					entity->hitbox_position(get_hitbox_by_hitgroup(hitgroup), g_ctx.globals.aimbot_matrix[user_id]),
					m_globals()->m_curtime,
					Color::White,
					damage,
					hitgroup
	...
	else if (!strcmp(event_name, crypt_str("round_start")))
	{
		...
		for (auto i = 1; i < m_globals()->m_maxclients; i++)
		{
			...
			g_ctx.globals.check_aa_enable[i] = true;
			g_ctx.globals.check_side[i] = SIDE_RESOLVER_FIRST;
			g_ctx.globals.check_side_type[i] = false;
			g_ctx.globals.lby[i] = false;
