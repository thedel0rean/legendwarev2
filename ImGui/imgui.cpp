...
//-----------------------------------------------------------------------------
float clip_x(float n, float lower, float upper)
{
...
void ImGui::NewFrame() {
	...
	if (g.HoveredWindow && g.IO.MouseWheel != 0.0f && !g.HoveredWindow->Collapsed) {
		...
		if (g.IO.KeyCtrl && g.IO.FontAllowUserScaling) {
			...
			SetWindowScrollY(window, window->Scroll.y - ((g.IO.MouseWheel * 0.15f) * window->CalcFontSize() * scroll_lines * 2.2f));
		}
	}
...
bool ImGui::BeginPopupContextWindow(const char* str_id, int mouse_button, bool also_over_items) {
	...
	if (IsMouseReleased(mouse_button) && IsWindowHovered())
...
bool ImGui::ListBoxConfigArray(const char* label, int* currIndex, std::vector<std::string>& values, int alpha, int height)
{
	return ImGui::ListBox(label, currIndex, vector_getter,
		static_cast<void*>(&values), values.size(), height, alpha);
}
void ImGui::EndChild(int alpha, bool namep) {
	ImGuiWindow* window = GetCurrentWindow();
	...
	if (namep)
	{
		if (window->ScrollbarY) {
			...
			auto arrow = [window](int x, int y, bool normal, ImColor color) {
				if (normal) {
					for (auto i = 5; i >= 3; --i) {
						auto offset = 5 - i;

						window->DrawList->AddLine(ImVec2(x + offset, y + offset), ImVec2(x + offset + nigga(i - offset, 0, 5), y + offset), color);
					}
				}
				else {
					window->DrawList->AddLine(ImVec2(x + 2, y), ImVec2(x + 3, y), color);
					window->DrawList->AddLine(ImVec2(x + 1, y + 1), ImVec2(x + 4, y + 1), color);
					window->DrawList->AddLine(ImVec2(x, y + 2), ImVec2(x + 5, y + 2), color);
				}
			};

			if (window->Scroll.y > 1.0f) {
				window->DrawList->AddRectFilledMultiColor(min_top - ImVec2(7, 0), max_top - ImVec2(1, 0), ImColor(0, 0, 0, alpha * 0.39), ImColor(0, 0, 0, alpha * 0.39), ImColor(0, 0, 0, 0), ImColor(0, 0, 0, 0));
				arrow(max_top.x - 20, max_top.y - 8, false, ImColor(150, 150, 150, alpha));
			}
			if ((GetScrollMaxY() - window->Scroll.y) > 1.f) {
				window->DrawList->AddRectFilledMultiColor(min_bot - ImVec2(7, 0), max_bot - ImVec2(1, 0), ImColor(0, 0, 0, 0), ImColor(0, 0, 0, 0), ImColor(0, 0, 0, alpha * 0.39), ImColor(0, 0, 0, alpha * 0.39));
				arrow(max_bot.x - 20, max_bot.y - 10, true, ImColor(150, 150, 150, alpha));
			}
		}
	}
	...
	if (namep)
	{
		...
		ImGui::GetWindowDrawList()->AddText(ImVec2(text_position.x - 1, text_position.y - 1), IM_COL32(0, 0, 0, alpha), window_title.c_str());
		ImGui::GetWindowDrawList()->AddText(ImVec2(text_position.x - 1, text_position.y), IM_COL32(0, 0, 0, alpha), window_title.c_str());
		ImGui::GetWindowDrawList()->AddText(ImVec2(text_position.x - 1, text_position.y + 1), IM_COL32(0, 0, 0, alpha), window_title.c_str());
		ImGui::GetWindowDrawList()->AddText(ImVec2(text_position.x, text_position.y - 1), IM_COL32(0, 0, 0, alpha), window_title.c_str());
		ImGui::GetWindowDrawList()->AddText(ImVec2(text_position.x, text_position.y + 1), IM_COL32(0, 0, 0, alpha), window_title.c_str());
		ImGui::GetWindowDrawList()->AddText(ImVec2(text_position.x + 1, text_position.y - 1), IM_COL32(0, 0, 0, alpha), window_title.c_str());
		ImGui::GetWindowDrawList()->AddText(ImVec2(text_position.x + 1, text_position.y), IM_COL32(0, 0, 0, alpha), window_title.c_str());
		ImGui::GetWindowDrawList()->AddText(ImVec2(text_position.x + 1, text_position.y + 1), IM_COL32(0, 0, 0, alpha), window_title.c_str());
		ImGui::GetWindowDrawList()->AddText(text_position, IM_COL32(255, 255, 255, alpha), window_title.c_str());

		PopClipRect();
	}

	IM_ASSERT(window->Flags & ImGuiWindowFlags_ChildWindow); // mismatched error
...
void ImGui::EndChildFrame(bool namep) {
	ImGui::EndChild(255, namep);
	ImGui::PopStyleVar(2);
	ImGui::PopStyleColor();
}
...
void ImGui::PushStyleColor(ImGuiCol idx, ImColor col)
{
	...
	backup.BackupValue = g.Style.Colors[idx];
	g.ColorModifiers.push_back(backup);
	g.Style.Colors[idx] = ImVec4(col.Value.x, col.Value.y, col.Value.z, col.Value.z);
}
...
void ImGui::SetScrollHere(float center_y_ratio, float y_offset) {
	ImGuiWindow* window = GetCurrentWindow();
...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags, const char* keybind_format) {
	ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
	...
	float deltatime = m_globals()->m_frametime * 65.f / 120.f;

	static std::map<ImGuiID, ButtonState> press_anim;
	auto it_press = press_anim.find(id);
	if (it_press == press_anim.end())
	{
		ButtonState newstate;
		newstate.alpha = 0.f;
		newstate.state = false;
		press_anim.insert({ id, newstate });
		it_press = press_anim.find(id);
	}
	if (pressed || it_press->second.state)
	{
		it_press->second.state = true;
		it_press->second.alpha = ImMin(it_press->second.alpha + 1.f * deltatime, 0.15f);
		if (it_press->second.alpha == 0.15f)
			it_press->second.state = false;
	}
	else
		it_press->second.alpha = ImMax(0.f, it_press->second.alpha - 1.f * deltatime);

	static std::map<ImGuiID, float> hover_anim;
	auto it = hover_anim.find(id);
	if (it == hover_anim.end())
	{
		static float value = 0.f;
		hover_anim.insert({ id, value });
		it = hover_anim.find(id);
	}
	if (hovered)
		it->second = ImMin(it->second + 1.f * deltatime, 0.25f);
	else
		it->second = ImMax(0.f, it->second - 1.f * deltatime);

    // Render
	const ImU32 col_29 = GetColorU32(ImVec4(0.11f, 0.11f, 0.11f, 1.f));
    const ImU32 col_19 = GetColorU32(ImVec4(0.074f, 0.074f, 0.074f, 1.f));
    //RenderFrame(bb.Min, bb.Max, col, true, style.FrameRounding);
	window->DrawList->AddRectFilledMultiColor(bb.Min, bb.Max, col_29, col_29, col_19, col_19);
	window->DrawList->AddRectFilled(bb.Min, bb.Max, GetColorU32(ImVec4(0.45f, 0.45f, 0.45f, it_press->second.alpha))); // anim
	window->DrawList->AddRect(bb.Min, bb.Max, ImColor(255, 255, 255, 18));
	window->DrawList->AddRect(bb.Min, bb.Max, GetColorU32(ImVec4(0.8f, 0.8f, 0.8f, it->second))); // anim hover
...
bool ImGui::ButtonEx(const char* label, const char* icon, ImFont * iconfont, float x_add, const ImVec2& size_arg, ImGuiButtonFlags flags) {
	ImGuiWindow* window = GetCurrentWindow();
	if (window->SkipItems)
		return false;
	...
	if ((flags & ImGuiButtonFlags_AlignTextBaseLine) && style.FramePadding.y < window->DC.CurrentLineTextBaseOffset) // Try to vertically align buttons that are smaller/have no padding so that text baseline matches (bit hacky, since it shouldn't be a flag)
		pos.y += window->DC.CurrentLineTextBaseOffset - style.FramePadding.y;
	ImVec2 size = CalcItemSize(size_arg, label_size.x + style.FramePadding.x * 2.0f, label_size.y + style.FramePadding.y * 2.0f);
	ImVec2 size_icon = CalcItemSize(size_arg, icon_size.x + style.FramePadding.x * 2.0f, icon_size.y + style.FramePadding.y * 2.0f);

	const ImRect bb(pos, pos + size);
	const ImRect bb_icon(pos, pos + size_icon);
	ItemSize(size, style.FramePadding.y);
	if (!ItemAdd(bb, &id))
		return false;
	...
	RenderTextClipped(bb_icon.Min + style.FramePadding, bb_icon.Max - style.FramePadding, icon, NULL, &icon_size, ImVec2(x_add, 0.3f), &bb_icon);
	ImGui::PopFont();
	RenderTextClipped(bb.Min + style.FramePadding, bb.Max - style.FramePadding, label, NULL, &label_size, ImVec2(0.5f, 0.7f), &bb);
...
bool ImGui::Button(const char* label, const char* unique_id, const ImVec2& size_arg) {
	ImGuiWindow* window = GetCurrentWindow();
	if (window->SkipItems)
		return false;
...
bool ImGui::SemiTab(const char* label, const ImVec2& size_arg, bool active) {
	ImGuiWindow* window = GetCurrentWindow();
	if (window->SkipItems)
		return false;
	...
	static std::map<ImGuiID, ImColor> active_anim;
	auto it_active = active_anim.find(id);
	if (it_active == active_anim.end())
	{
		static ImColor value = ImColor(210, 210, 210, 255);
		active_anim.insert({ id, value });
		it_active = active_anim.find(id);
	}
	if (active)
	{
		ImColor to = ImColor((int)(g_ctx.globals.menu_color.x * 255.f), (int)(g_ctx.globals.menu_color.y * 255.f), (int)(g_ctx.globals.menu_color.z * 255.f), 255);
		if (it_active->second.Value.x != to.Value.x)
		{
			if (it_active->second.Value.x < to.Value.x)
				it_active->second.Value.x = ImMin(it_active->second.Value.x + 1.f * deltatime, to.Value.x);
			else if (it_active->second.Value.x > to.Value.x)
				it_active->second.Value.x = ImMax(to.Value.x, it_active->second.Value.x - 1.f * deltatime);
		}

		if (it_active->second.Value.y != to.Value.y)
		{
			if (it_active->second.Value.y < to.Value.y)
				it_active->second.Value.y = ImMin(it_active->second.Value.y + 1.f * deltatime, to.Value.y);
			else if (it_active->second.Value.y > to.Value.y)
				it_active->second.Value.y = ImMax(to.Value.y, it_active->second.Value.y - 1.f * deltatime);
		}

		if (it_active->second.Value.z != to.Value.z)
		{
			if (it_active->second.Value.z < to.Value.z)
				it_active->second.Value.z = ImMin(it_active->second.Value.z + 1.f * deltatime, to.Value.z);
			else if (it_active->second.Value.z > to.Value.z)
				it_active->second.Value.z = ImMax(to.Value.z, it_active->second.Value.z - 1.f * deltatime);
		}
	}
	else
	{
		ImColor to = ImColor(210, 210, 210, 255);
		if (it_active->second.Value.x != to.Value.x)
		{
			if (it_active->second.Value.x < to.Value.x)
				it_active->second.Value.x = ImMin(it_active->second.Value.x + 1.f * deltatime, to.Value.x);
	...
	ImGui::PushStyleColor(ImGuiCol_Text, ImVec4(it_active->second.Value.x, it_active->second.Value.y, it_active->second.Value.z, 1.f));
...
bool ImGui::SemiTabRage(const char* label, const ImVec2& size_arg, bool active) {
	ImGuiWindow* window = GetCurrentWindow();
	if (window->SkipItems)
		return false;
	...
	if (active || hovered)
	{
		ImColor to = ImColor((int)(g_ctx.globals.menu_color.x * 255.f), (int)(g_ctx.globals.menu_color.y * 255.f), (int)(g_ctx.globals.menu_color.z * 255.f), hovered ? 70 : 255);
		if (it_active->second.Value.x != to.Value.x)
		{
			if (it_active->second.Value.x < to.Value.x)
				it_active->second.Value.x = ImMin(it_active->second.Value.x + 1.f * deltatime, to.Value.x);
	...
	if (label == "J")
	{
		ImGui::PushStyleColor(ImGuiCol_Text, ImVec4(it_active->second.Value.x, it_active->second.Value.y, it_active->second.Value.z, 0.4f));
		RenderTextClipped(ImVec2(bb.Min.x + style.FramePadding.x + 28, bb.Min.y + style.FramePadding.y + 3), bb.Max - style.FramePadding, "A", NULL, &label_size, style.ButtonTextAlign, &bb);
		ImGui::PopStyleColor();
	}
...
bool ImGui::Button(const char* label, const ImVec2& size_arg, const char* keybind_format) {
	return ButtonEx(label, size_arg, 0, keybind_format);
}
bool ImGui::Button(const char* label, const char* icon, ImFont * iconfont, float x_add, const ImVec2& size_arg) {
	return ButtonEx(label, icon, iconfont, x_add, size_arg, 0);
}
...
bool ImGui::SliderBehavior(const ImRect& frame_bb, ImGuiID id, const char* display_format, float* v, float v_min, float v_max, float power, int decimal_precision, ImGuiSliderFlags flags) {
	...
	ImGui::PushStyleVar(style.FrameRounding, 3.f);
	...
	//window->DrawList->AddRect(frame_bb.Min - ImVec2(0, 1), frame_bb.Max - ImVec2(0, 3), GetColorU32(ImVec4(g_ctx.globals.menu_color.x, g_ctx.globals.menu_color.y, g_ctx.globals.menu_color.z, it_alpha->second)), style.FrameRounding);
...
//   "Gold: %.0f"   Gold: 1
bool ImGui::SliderFloat(const char* label, float* v, float v_min, float v_max, bool none_zero, bool minimum_damage, const char* display_format, float power) {
	ImGuiWindow* window = GetCurrentWindow();
	if (window->SkipItems)
		return false;
	...
	const ImVec2 label_size = CalcTextSize(label, NULL, true);
	const ImRect frame_bb(window->DC.CursorPos - ImVec2(0, 2), window->DC.CursorPos + ImVec2(w, label_size.y + style.FramePadding.y* 0.f - 6) + ImVec2(0, 1));
	...
	float deltatime = m_globals()->m_frametime * 65.f / 52.f;

	static std::map<ImGuiID, ImColor> alpha_anim;
	auto it_alpha = alpha_anim.find(id);
	if (it_alpha == alpha_anim.end())
	{
		static ImColor value = ImColor(210, 210, 210, 255);
		alpha_anim.insert({ id, value });
		it_alpha = alpha_anim.find(id);
	}
	if (g.ActiveId == id)
	{
		ImColor to = ImColor((int)(g_ctx.globals.menu_color.x * 255.f), (int)(g_ctx.globals.menu_color.y * 255.f), (int)(g_ctx.globals.menu_color.z * 255.f), 255);
		if (it_alpha->second.Value.x != to.Value.x)
		{
			if (it_alpha->second.Value.x < to.Value.x)
				it_alpha->second.Value.x = ImMin(it_alpha->second.Value.x + 1.f * deltatime, to.Value.x);
			else if (it_alpha->second.Value.x > to.Value.x)
				it_alpha->second.Value.x = ImMax(to.Value.x, it_alpha->second.Value.x - 1.f * deltatime);
		}

		if (it_alpha->second.Value.y != to.Value.y)
		{
			if (it_alpha->second.Value.y < to.Value.y)
				it_alpha->second.Value.y = ImMin(it_alpha->second.Value.y + 1.f * deltatime, to.Value.y);
			else if (it_alpha->second.Value.y > to.Value.y)
				it_alpha->second.Value.y = ImMax(to.Value.y, it_alpha->second.Value.y - 1.f * deltatime);
		}

		if (it_alpha->second.Value.z != to.Value.z)
		{
			if (it_alpha->second.Value.z < to.Value.z)
				it_alpha->second.Value.z = ImMin(it_alpha->second.Value.z + 1.f * deltatime, to.Value.z);
			else if (it_alpha->second.Value.z > to.Value.z)
				it_alpha->second.Value.z = ImMax(to.Value.z, it_alpha->second.Value.z - 1.f * deltatime);
		}
	}	
	else
	{
	...
	if (none_zero && !*v)
		value_buf_end = value_buf + ImFormatString(value_buf, IM_ARRAYSIZE(value_buf), "%s", "None");
	else if (minimum_damage && *v > 100)
	{
		auto value_damage = "HP + " + std::to_string((int)*v - 100);
		value_buf_end = value_buf + ImFormatString(value_buf, IM_ARRAYSIZE(value_buf), "%s", value_damage.c_str());
	}

	ImGui::PushStyleColor(ImGuiCol_Text, ImVec4(it_alpha->second.Value.x, it_alpha->second.Value.y, it_alpha->second.Value.z, 1.f));
...
bool ImGui::SliderAngle(const char* label, float* v_rad, float v_degrees_min, float v_degrees_max) {
	...
	bool value_changed = SliderFloat(label, &v_deg, v_degrees_min, v_degrees_max, false, false, "%.0f deg", 1.0f);
	*v_rad = v_deg * (2 * IM_PI) / 360.0f;
	return value_changed;
}

bool ImGui::SliderInt(const char* label, int* v, int v_min, int v_max, bool minimum_damage, const char* display_format)
{
	if (!display_format)
		display_format = "%.0f";

	auto v_f = (float)*v;
	auto value_changed = SliderFloat(label, &v_f, (float)v_min, (float)v_max, false, minimum_damage, display_format, 1.0f);
...
bool ImGui::SliderFloatN(const char* label, float* v, int components, float v_min, float v_max, const char* display_format, float power) {
	...
	for (int i = 0; i < components; i++) {
		PushID(i);
		value_changed |= SliderFloat("##v", &v[i], v_min, v_max, false, false, display_format, power);
...
int processint(int from, int to, int value)
{
	auto delta = m_globals()->m_frametime;
	...
	if (from < to)
	{
		value = ImClamp(value + (int)(2 * delta), 0, 255);
	}
	else
	{
		value = ImClamp(value - (int)(2 * delta), 0, 255);
...
bool ImGui::Checkbox(const char* label, bool* v) {
	...
	const ImRect check_bb(window->DC.CursorPos - ImVec2(1, 1), window->DC.CursorPos + ImVec2(1, 1) + ImVec2(label_size.y + style.FramePadding.y * -1, label_size.y + style.FramePadding.y * -1));
	...
	if (hovered)
		it->second = ImMin(it->second + 1.f * deltatime, 0.55f);
	else
		it->second = ImMax(0.f, it->second - 1.f * deltatime);


	static std::map<ImGuiID, CheckboxState> press_anim;
	auto it_press = press_anim.find(id);
	if (it_press == press_anim.end())
	{
		CheckboxState newstate;
		newstate.alpha_anim = *v ? 1.f : 0.f;
		newstate.state = *v ? true : false;
		press_anim.insert({ id, newstate });
		it_press = press_anim.find(id);
	}
	if (*v != it_press->second.state)
	{
		if (*v)
			it_press->second.alpha_anim = ImMin(it_press->second.alpha_anim + 1.f * deltatime, 1.f);
		else
			it_press->second.alpha_anim = ImMax(0.f, it_press->second.alpha_anim - 1.f * deltatime);
		if ((*v && it_press->second.alpha_anim == 1.f) || (!(*v) && it_press->second.alpha_anim == 0.f))
			it_press->second.state = *v;
	}
	else
		it_press->second.alpha_anim = *v ? 1.f : 0.f;

	window->DrawList->AddRectFilled(check_bb.Min + ImVec2(pad, pad), check_bb.Max - ImVec2(pad, pad), GetColorU32(ImVec4(0 / 255.f, 0 / 255.f, 0 / 255.f, 0.1f)), style.FrameRounding);
	...
	if (!(*v))
		window->DrawList->AddRect(check_bb.Min - ImVec2(1.f, 1.f), check_bb.Max + ImVec2(1.f, 1.f), GetColorU32(ImVec4(g_ctx.globals.menu_color.x, g_ctx.globals.menu_color.y, g_ctx.globals.menu_color.z, it->second)), style.FrameRounding); // anim
	
	window->DrawList->AddRectFilled(check_bb.Min + ImVec2(pad, pad), check_bb.Max - ImVec2(pad, pad), GetColorU32(ImVec4(g_ctx.globals.menu_color.x, g_ctx.globals.menu_color.y, g_ctx.globals.menu_color.z, it_press->second.alpha_anim)), style.FrameRounding); // fill
	if (*v) {
	...
	if (label_size.x > 0.0f) {
		...
		RenderText(text_bb.GetTL() - ImVec2(0, 5.25f) + ImVec2(0, 0.25f), label);
		PopStyleColor();
	}
...
bool ImGui::InputTextEx(const char* label, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiTextEditCallback callback, void* user_data, bool can_focus) 
{
	ImGuiWindow* window = GetCurrentWindow();
	if (window->SkipItems)
		return false;
	...
	const ImRect frame_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(w, size.y));
	const ImRect total_bb(frame_bb.Min, frame_bb.Max + ImVec2(label_size.x > 0.0f ? (style.ItemInnerSpacing.x + label_size.x) : 0.0f, 0.0f));
	...
	if (can_focus && (focus_requested || user_clicked || user_scrolled))
	{
		if (g.ActiveId != id) 
		{
			// Start edition
	...
	char* buf_display = (g.ActiveId == id && is_editable) ? edit_state.TempTextBuffer.Data : buf; buf = NULL;
	std::string str = buf_display;

	while (str.find("ё") != std::string::npos)
		str.replace(str.find("ё"), 2, "е");

	strcpy_s(buf_display, buf_size, str.c_str());
...
bool ImGui::InputText(const char* label, char* buf, size_t buf_size, ImGuiInputTextFlags flags, ImGuiTextEditCallback callback, void* user_data, bool can_focus) 
{
	IM_ASSERT(!(flags & ImGuiInputTextFlags_Multiline)); // call InputTextMultiline()
	return InputTextEx(label, buf, (int)buf_size, ImVec2(0, 0), flags, callback, user_data, can_focus);
}
...
bool ImGui::InputScalarEx(const char* label, ImGuiDataType data_type, void* data_ptr, void* step_ptr, void* step_fast_ptr, const char* scalar_format, ImGuiInputTextFlags extra_flags, bool can_focus)
{
	ImGuiWindow* window = GetCurrentWindow();
	if (window->SkipItems)
		return false;
	...
	if (InputText("", buf, IM_ARRAYSIZE(buf), extra_flags, 0, 0, can_focus)) // PushId(label) + "" gives us the expected ID from outside point of view
	...
	if (step_ptr) {
		PopItemWidth();
		SameLine(0, style.ItemInnerSpacing.x);
		if (ButtonEx("-", button_sz, ImGuiButtonFlags_Repeat | ImGuiButtonFlags_DontClosePopups, "None") && *(int*)(data_ptr) > 0)
		{
			DataTypeApplyOp(data_type, '-', data_ptr, g.IO.KeyCtrl && step_fast_ptr ? step_fast_ptr : step_ptr);
			value_changed = true;
		}
		SameLine(0, style.ItemInnerSpacing.x);
		if (ButtonEx("+", button_sz, ImGuiButtonFlags_Repeat | ImGuiButtonFlags_DontClosePopups, "None")) 
		{
			DataTypeApplyOp(data_type, '+', data_ptr, g.IO.KeyCtrl && step_fast_ptr ? step_fast_ptr : step_ptr);
			value_changed = true;
		}
...
bool ImGui::InputInt(const char* label, int* v, int step, int step_fast, ImGuiInputTextFlags extra_flags, bool can_focus) {
	if (*v < 0)
		*v = 0;

	// Hexadecimal input provided as a convenience but the flag name is awkward. Typically you'd use InputText() to parse your own data, if you want to handle prefixes.
	const char* scalar_format = (extra_flags & ImGuiInputTextFlags_CharsHexadecimal) ? "%08X" : "%d";
	return InputScalarEx(label, ImGuiDataType_Int, (void*)v, (void*)(step > 0.0f ? &step : NULL), (void*)(step_fast > 0.0f ? &step_fast : NULL), scalar_format, extra_flags, can_focus);
}
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImVec2 popup_size)
{
	...
	const ImVec2 label_size = CalcTextSize(label, NULL, true);
	const ImRect frame_bb(window->DC.CursorPos + ImVec2(0, 0), window->DC.CursorPos + ImVec2(w, label_size.y + style.FramePadding.y*2.0f - 1));
	const ImRect total_bb(frame_bb.Min, frame_bb.Max + ImVec2(label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f, 0.0f));
	ItemSize(total_bb, style.FramePadding.y);
	if (!ItemAdd(total_bb, &id))
		return false;
	...
	if (hovered || IsPopupOpen(id))
		it_alpha->second = ImMin(it_alpha->second + 1.f * deltatime, 0.55f);
	else
		it_alpha->second = ImMax(0.f, it_alpha->second - 1.f * deltatime);

	const ImRect value_bb(frame_bb.Min, frame_bb.Max - ImVec2(arrow_size, 0.0f));
	RenderFrame(frame_bb.Min, frame_bb.Max, GetColorU32(ImGuiCol_FrameBg), true, style.FrameRounding);
	window->DrawList->AddRectFilled(frame_bb.Min - ImVec2(1, 1), frame_bb.Max + ImVec2(1, 1), GetColorU32(ImVec4(0 / 255.f, 0 / 255.f, 0 / 255.f, 0.1f)), style.FrameRounding);
	window->DrawList->AddRectFilled(frame_bb.Min, frame_bb.Max, GetColorU32(ImGuiCol_FrameBg), style.FrameRounding);
	window->DrawList->AddRect(frame_bb.Min, frame_bb.Max, GetColorU32(ImVec4(g_ctx.globals.menu_color.x, g_ctx.globals.menu_color.y, g_ctx.globals.menu_color.z, it_alpha->second)), style.FrameRounding);
	...
	if (preview_value != NULL)
		RenderTextClipped(frame_bb.Min + style.FramePadding - ImVec2(-2, 2), value_bb.Max, preview_value, NULL, NULL, ImVec2(0.0f, 0.0f));
...
bool ImGui::Combo(const char* label, int* current_item, bool(*items_getter)(void*, int, const char**), void* data, int items_count, int height_in_items) {
	...
	if (IsPopupOpen(id)) {
		...
		ImRect popup_rect(ImVec2(frame_bb.Min.x + 1, popup_y1 - 3), ImVec2(frame_bb.Max.x - 1, popup_y2));
		SetNextWindowPos(popup_rect.Min + ImVec2(-1, 3));
		SetNextWindowSize(popup_rect.GetSize() + ImVec2(2, -(ImMin(items_count, height_in_items) * 2)));
		PushStyleVar(ImGuiStyleVar_WindowPadding, ImVec2(0, 0));

		const ImGuiWindowFlags flags = ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_ComboBox | ((window->Flags & ImGuiWindowFlags_ShowBorders) ? ImGuiWindowFlags_ShowBorders : 0);
		...
		if (BeginPopupEx(id, flags)) 
		{
			for (auto i = 0; i < items_count; i++) 
			{
				...
				if (Selectable(item_text, item_selected, items_count - i == 1)) 
				{
					ClearActiveID();
					value_changed = true;
					*current_item = i;
...
bool ImGui::SelectableMeme(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg, int alpha) {
	ImGuiWindow* window = GetCurrentWindow();
	if (window->SkipItems)
		return false;
	...
	label_size = label_size - ImVec2(0, 2);
	...
	float deltatime = m_globals()->m_frametime * 65.f / 52.f * 1.4f;
	...
	{
		ImColor to = ImColor(255, 255, 255, 255);
		if (it_alpha->second.Value.x != to.Value.x)
		{
			if (it_alpha->second.Value.x < to.Value.x)
				it_alpha->second.Value.x = ImMin(it_alpha->second.Value.x + 1.f * deltatime, to.Value.x);
	...
	if (alpha < 254)
	{
		ImGui::PushStyleColor(ImGuiCol_Text, ImVec4(it_alpha->second.Value.x, it_alpha->second.Value.y, it_alpha->second.Value.z, 255 - alpha));
		ImGui::PushStyleColor(ImGuiCol_TextDisabled, ImVec4(it_alpha->second.Value.x, it_alpha->second.Value.y, it_alpha->second.Value.z, 255 - alpha));
		RenderTextClipped(ImVec2(bb.Min.x + 6, bb.Min.y), bb_with_spacing.Max, label, NULL, &label_size, ImVec2(0.f, 0.5f));
...
// Tip: pass an empty label (e.g. "##dummy") then you can use the space to draw other text or image.
// But you need to make sure the ID is unique, e.g. enclose calls in PushID/PopID.
bool ImGui::Selectable(const char* label, bool selected, bool last, ImGuiSelectableFlags flags, const ImVec2& size_arg) {
	ImGuiWindow* window = GetCurrentWindow();
	if (window->SkipItems)
		return false;
	...
	ImGui::PushStyleColor(ImGuiCol_Text, it_alpha->second.Value);
	RenderTextClipped(bb.Min, bb_with_spacing.Max, label, NULL, &label_size, ImVec2(0.47f, 0.5f));
	ImGui::PopStyleColor();

	window->DrawList->AddLine(bb.Min, ImVec2(bb_with_spacing.Min.x, bb_with_spacing.Max.y), ImColor(255, 255, 255, 18));
	window->DrawList->AddLine(ImVec2(bb_with_spacing.Min.x + 174, bb.Min.y), ImVec2(bb_with_spacing.Min.x + 174, bb_with_spacing.Max.y), ImColor(255, 255, 255, 18));
	if (last)
	{
		window->DrawList->AddLine(ImVec2(bb.Min.x, bb_with_spacing.Max.y), ImVec2(bb_with_spacing.Min.x, bb_with_spacing.Max.y + 3), ImColor(255, 255, 255, 18));
		window->DrawList->AddLine(ImVec2(bb_with_spacing.Min.x + 174, bb_with_spacing.Max.y), ImVec2(bb_with_spacing.Min.x + 174, bb_with_spacing.Max.y + 3), ImColor(255, 255, 255, 18));
		window->DrawList->AddLine(ImVec2(bb_with_spacing.Min.x, bb_with_spacing.Max.y + 3), ImVec2(bb_with_spacing.Min.x + 174, bb_with_spacing.Max.y + 3), ImColor(255, 255, 255, 18));
	}

	if (!last)
		window->DrawList->AddLine(ImVec2(bb_with_spacing.Min.x + 4, bb_with_spacing.Max.y + 2), ImVec2(bb_with_spacing.Min.x + 174 - 4, bb_with_spacing.Max.y + 2), ImColor(255, 255, 255, 7));
...
bool ImGui::Selectable(const char* label, bool* p_selected, bool last, ImGuiSelectableFlags flags, const ImVec2& size_arg) {
	if (Selectable(label, *p_selected, last, flags, size_arg)) {
...
bool ImGui::ListBoxHeader(const char* label, const ImVec2& size_arg, int alpha) {
	ImGuiWindow* window = GetCurrentWindow();
	if (window->SkipItems)
		return false;
	...
	ImGui::GetWindowDrawList()->AddRectFilled(ImVec2(bb.Min.x, bb.Min.y - 15), bb.Max, ImColor(17, 17, 17, alpha));
	return true;
}

bool ImGui::ListBoxHeader(const char* label, int items_count, int height_in_items, int alpha) {
	// Size default to hold ~7 items. Fractional number of items helps seeing that we can scroll down/up without looking at scrollbar.
	...
	ImVec2 size;
	size.x = 0.0f;
	size.y = GetTextLineHeightWithSpacing() * (float)height_in_items + GetStyle().ItemSpacing.y;
	return ListBoxHeader(label, size, alpha);
}
...
bool ImGui::ListBox(const char* label, int* current_item, bool(*items_getter)(void*, int, const char**), void* data, int items_count, int height_in_items, int alpha) {
	if (!ListBoxHeader(label, items_count, height_in_items, alpha))
		return false;
	...
	while (clipper.Step())
	{
		for (int i = clipper.DisplayStart; i < clipper.DisplayEnd; i++) 
		{
			...
			if (SelectableMeme(item_text, item_selected, 0, ImVec2(0, 0), alpha)) 
			{
				*current_item = i;
				value_changed = true;
			}
...
bool ImGui::MenuItem(const char* label, const char* shortcut, bool selected, bool enabled) {
	...
	bool pressed = Selectable(label, false, false, ImGuiSelectableFlags_MenuItem | ImGuiSelectableFlags_DrawFillAvailWidth | (enabled ? 0 : ImGuiSelectableFlags_Disabled), ImVec2(w, 0.0f));
...
bool ImGui::BeginMenu(const char* label, bool enabled) {
	...
	if (window->DC.LayoutType == ImGuiLayoutType_Horizontal) {
		...
		pressed = Selectable(label, menu_is_open, false, ImGuiSelectableFlags_Menu | ImGuiSelectableFlags_DontClosePopups | (!enabled ? ImGuiSelectableFlags_Disabled : 0), ImVec2(w, 0.0f));
		PopStyleVar();
		SameLine();
		window->DC.CursorPos.x += (float)(int)(style.ItemSpacing.x * 0.5f);
	}
		...
		float extra_w = ImMax(0.0f, GetContentRegionAvail().x - w);
		pressed = Selectable(label, menu_is_open, false, ImGuiSelectableFlags_Menu | ImGuiSelectableFlags_DontClosePopups | ImGuiSelectableFlags_DrawFillAvailWidth | (!enabled ? ImGuiSelectableFlags_Disabled : 0), ImVec2(w, 0.0f));
...
bool ImGui::ColorButton(const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags, ImVec2 size) {
	...
	{
		RenderColorRectWithAlphaCheckerboard(bb_inner.Min, bb_inner.Max, GetColorU32(col), grid_step, ImVec2(off, off), rounding);
/*
		float mid_x = (float)(int)((bb_inner.Min.x + bb_inner.Max.x) * 0.5f + 0.5f);
		RenderColorRectWithAlphaCheckerboard(ImVec2(bb_inner.Min.x + grid_step, bb_inner.Min.y), bb_inner.Max, GetColorU32(col), grid_step, ImVec2(-grid_step + off, off), rounding, ImGuiCorner_TopRight | ImGuiCorner_BottomRight);
		window->DrawList->AddRectFilled(bb_inner.Min, ImVec2(mid_x, bb_inner.Max.y), GetColorU32(col_without_alpha), rounding, ImGuiCorner_TopLeft | ImGuiCorner_BottomLeft);
*/
	}
	else
	{
		...
		if (col_source.w < 1.0f)
			...
			window->DrawList->AddRectFilled(bb_inner.Min, bb_inner.Max, GetColorU32(col_source), rounding, ImGuiCorner_All);
	}
...
static void ColorPickerOptionsPopup(ImGuiColorEditFlags flags, float* ref_col) {
	...
	if (allow_opt_picker) {
		...
		for (int picker_type = 0; picker_type < 2; picker_type++) {
			...
			if (ImGui::Selectable("##selectable", false, false, 0, picker_size)) // By default, Selectable() is closing popup
...
bool ImGui::ColorEdit4(const char* label, Color* newcolor, ImGuiColorEditFlags flags) {
	ImGui::SameLine();
	ImGui::SetCursorPos(ImVec2(ImGui::GetWindowSize().x - 32.0f - ImGui::GetStyle().FramePadding.x * 2.0f, ImGui::GetCursorPosY() + 3));
	...
	float col[4] =
	{
		(float)(*newcolor).r() / 255.0f,
		(float)(*newcolor).g() / 255.0f, 
		(float)(*newcolor).b() / 255.0f,
		(float)(*newcolor).a() / 255.0f 
	};
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col) {
	...
	if (!(flags & ImGuiColorEditFlags_NoSidePreview))
	{
		...
		ImGui::BeginGroup();
		auto square_sz = g.FontSize + g.Style.FramePadding.y * 2.0f;
		ImVec4 col_v4(col[0], col[1], col[2], (flags & ImGuiColorEditFlags_NoAlpha) ? 1.0f : col[3]);
		if ((flags & ImGuiColorEditFlags_NoLabel))
			Text("Current");
		ColorButton("##current", col_v4, (flags & (ImGuiColorEditFlags_HDR | ImGuiColorEditFlags_AlphaPreviewHalf | ImGuiColorEditFlags_NoTooltip)), ImVec2(square_sz * 3, square_sz * 2));
		if (ref_col != NULL)
		{
			Text("Original");
			ImVec4 ref_col_v4(ref_col[0], ref_col[1], ref_col[2], alpha_bar ? ref_col[3] : 1.f);
			if (ColorButton("##original", ref_col_v4, (flags & (ImGuiColorEditFlags_HDR | ImGuiColorEditFlags_AlphaPreviewHalf | ImGuiColorEditFlags_NoTooltip)), ImVec2(square_sz * 3, square_sz * 2)))
			{
					col[0] = ref_col_v4.x;
					col[1] = ref_col_v4.y;
					col[2] = ref_col_v4.z;
...
void ImGui::Separator() {
	...
	const ImRect bb(ImVec2(x1, window->DC.CursorPos.y - 1.f), ImVec2(x2, window->DC.CursorPos.y));
	...
	window->DrawList->AddRectFilledMultiColor(bb.Min, ImVec2(bb.Max.x - ((x2 - x1) / 2), bb.Max.y), ImColor(210, 210, 210, 0), ImColor(210, 210, 210, 30), ImColor(210, 210, 210, 30), ImColor(210, 210, 210, 0));
	window->DrawList->AddRectFilledMultiColor(ImVec2(bb.Min.x + ((x2 - x1) / 2), bb.Min.y), bb.Max, ImColor(210, 210, 210, 30), ImColor(210, 210, 210, 0), ImColor(210, 210, 210, 0), ImColor(210, 210, 210, 30));
